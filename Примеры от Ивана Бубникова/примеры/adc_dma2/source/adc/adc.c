...
const uint16_t adc_pin_1 = GPIO_Pin_0;
const uint16_t adc_pin_2 = GPIO_Pin_1;

const RccClockCmd adc_rcc_cmd = RCC_APB2PeriphClockCmd;
const uint32_t adc_rcc = RCC_APB2Periph_ADC1;
ADC_TypeDef * const adc = ADC1;
const uint8_t adc_channel_1 = ADC_Channel_8;
const uint8_t adc_channel_2 = ADC_Channel_9;
...
static uint16_t adcConvertedValue[adc_channels];
...
void adc_init( void )
{
    ...
    gpioInitStructure.GPIO_Pin = adc_pin_1 | adc_pin_2;
    ...
    ADC_RegularChannelConfig( adc, adc_channel_1, 1, adc_sample_time );
    ADC_RegularChannelConfig( adc, adc_channel_2, 2, adc_sample_time );

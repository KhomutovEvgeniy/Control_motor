...
В данном случае SYSTICK_PRESCALER = 1000 * 72, т.к. частота в протеусе 1МГц
***************************************************************************************************/
#define SYSTICK_PRESCALER 1000 / 720
#define DELAY 1000
...
int32_t power( int32_t num, int32_t pow);

/***************************************************************************************************
Описание: Быстрое возведение числа в степень
Аргументы: num - число, которое возводим, pow - степень числа
Возврат: Результат возведения числа num в степень pow
Замечания: 
***************************************************************************************************/
int32_t power(int32_t num, int32_t pow) 
{
...
void intToStr( int16_t value, char buf[])
{
    ...
    { 
        buf[i] = value % (int)power(10, (5 - i)) / (int)power(10, (5 - i - 1)) + '0'; 
    }
}
